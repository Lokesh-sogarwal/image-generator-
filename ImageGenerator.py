...
def display_image(category):
    ...
    url = (f"https://api.unsplash.com/photos/random?query={category}&orientation=land"
           f"scape&client_id=1n7sSMtCh8Hs_MrBOjhQ1SygTDA-BJ550UdX3rwLYZQ")
    data = requests.get(url).json()
    img_data = requests.get(data["urls"]["regular"]).content

    photo = ImageTk.PhotoImage(Image.open(io.BytesIO(img_data)).resize((600, 400),
...
def create_gui():
    ...
    category_options = ["Choose Category", "Food", "Animals", "People", "Music", "Art", "Vehicles",
                        "Random"]
    ...
    generate_button = ttk.Button(text="Generate Image", state="disabled",
